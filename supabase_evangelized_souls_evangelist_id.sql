-- ============================================================
-- Rattache un evangeliste connu (compte utilisateur) au formulaire
-- public d'evangelisation, en plus du champ texte libre existant.
-- ============================================================
...
CREATE OR REPLACE FUNCTION submit_evangelized_soul(
  ...
  p_evangelist_id        TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO evangelized_souls (
    ...
    evangelist_id, status, created_at, updated_at
  ) VALUES (
    ...
    NULLIF(trim(coalesce(p_evangelist_id, '')), ''),
    'active', now(), now()
  );
