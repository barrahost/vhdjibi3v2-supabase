...
export default function SoulForm({ onClose, initialFullName, eventId }: { onClose?: () => void; initialFullName?: string; eventId?: string | null }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (canEditAdnFields && !formData.general.isUndecided && !formData.general.serviceFamilyId) { setStepError('La famille de service est obligatoire.'); return; }
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        ...
        {step === 3 && (
          <div className="space-y-3">
            ...
            {/* Famille de service — sans objet pour une âme indécise, qui reste suivie par l'ADN */}
            {canEditAdnFields && (
              formData.general.isUndecided ? (
                <p className="flex items-center gap-1.5 text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
                  <HelpCircle className="w-3.5 h-3.5 flex-shrink-0" />
                  Âme indécise : pas de famille de service pour l'instant, elle sera suivie par l'équipe ADN.
                </p>
              ) : (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
